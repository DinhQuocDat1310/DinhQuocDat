...
const firstWaySumToN = (n: number): number => {
	if (!checkValidNumber(n)) return -1;
	let sum = 0;
...
const secondWaySumToN = (n: number): number => {
	if (!checkValidNumber(n)) return -1;
	return (n * (n + 1)) / 2;
};

//Complexity: O(N) - Efficiency: Less efficient because uses more memory due to recursion
const thirdWaySumToN = (n: number): number => {
	if (!checkValidNumber(n)) return -1;
	return n === 1 ? 1 : n + thirdWaySumToN(n - 1);
};

//Check valid number
const checkValidNumber = (n: number): boolean => {
	if (!n || n < 0 || n % 1 !== 0) return false;
	return true;
};

const resultFirstWaySumToN: number = firstWaySumToN(5);
const resultSecondWaySumToN: number = secondWaySumToN(5);
const resultThirdWaySumToN: number = thirdWaySumToN(5);
